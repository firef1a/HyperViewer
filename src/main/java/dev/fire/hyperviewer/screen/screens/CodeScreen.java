package dev.fire.hyperviewer.screen.screens;

import com.google.common.collect.Lists;
import com.mojang.blaze3d.systems.RenderSystem;
import dev.fire.hyperviewer.FileManager;
import dev.fire.hyperviewer.HyperViewer;
import dev.fire.hyperviewer.screen.chat.ChatManager;
import dev.fire.hyperviewer.screen.utils.*;
import dev.fire.hyperviewer.screen.utils.screenWidgets.*;
...
public class CodeScreen extends Screen {
    ...
    protected void init() {
        ...
        this.toggleButton = new RenderableToggleableButton(textRenderer,Text.literal("⏻"), widgetX, widgetY, widgetSize,widgetSize, setAlpha(0xed5139, 1f), setAlpha(0xe87361, 1f), setAlpha(0x62e04f, 1f), setAlpha(0x82db74, 1f), HyperViewer.functionDataManager.enableCodeViewer, false);
       ...
       // this.searchBar.getText();
        ...
        this.functionEntryList = HyperViewer.functionDataManager.functionEntryArrayList;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (toggleButton.isPointInside(mouse)) {
            ...
            HyperViewer.functionDataManager.enableCodeViewer = toggleButton.enabled;
            ...
            HyperViewer.functionDataManager.clear();
            HyperViewer.functionFinder.checkedBlocks.clear();
            HyperViewer.functionDataManager.focusedFunctionEntry = 0;
            ...
            String codeData = "";
            int i = 0;
            for (FunctionEntry functionEntry : HyperViewer.functionDataManager.functionEntryArrayList) {
                codeData = codeData + functionEntry.rawJsonString;
                if (i != HyperViewer.functionDataManager.functionEntryArrayList.size()-1){
                    codeData = codeData + "\n";
                }
                i++;
            }
            ...
            if (FileManager.exists(saveName)) {
                try {
                    ...
                    for (String s : collection) {
                        HyperViewer.functionDataManager.addFunction(s);
                    }
            ...
            for (RenderableRectangleObject rect : this.listRect.siblings) {
                for (RenderableRectangleObject sibling : rect.siblings) {
                    if (sibling.isPointInside(mouse)) {
                        HyperViewer.functionDataManager.remove(sibling.clickID);
                        int focusedTabLength = HyperViewer.functionDataManager.functionEntryArrayList.size()-1;
                        if (focusedTabLength < HyperViewer.functionDataManager.focusedFunctionEntry) {
                            HyperViewer.functionDataManager.focusedFunctionEntry = focusedTabLength;
                        }
                        hasClicked = true;
                        break;
                    }
                ...
                if (rect.isPointInside(mouse)) {
                    HyperViewer.functionDataManager.focusedFunctionEntry = rect.clickID;
